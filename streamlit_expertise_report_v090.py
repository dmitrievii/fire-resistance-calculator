...
def _section_inputs_v090(report: Mapping[str, Any]) -> list[str]:
    ...
    if d and s:
        lines.extend([
            ...
            f"Учтён ряд круглых болтовых отверстий по нейтральной линии: $d$ = **{_v089._display(d)}**, шаг $s$ = **{_v089._display(s)}**.", "",
            "$$ \\alpha_h=\\frac{s}{s-d} $$", "",
        ])
        if alpha:
    ...
    if loads:
        ...
        lines.extend([
            ";  \n".join(rendered) + ".", "",
            "Оси отчёта: **z — сильная**, **y — слабая**, $M_x$ — крутящий момент; $Q_y$ и $Q_z$ сохраняются как самостоятельные компоненты без образования фиктивной результирующей.", "",
        ])
    else:
        lines.extend(["В текущем сохранённом состоянии нет ненулевых силовых факторов для вывода в основном отчёте. Полный вектор доступен в Audit.", ""])
...
def _slenderness_section(report: Mapping[str, Any]) -> list[str]:
    ...
    lines.extend(["$$ i=\\sqrt{\\frac{I}{A}},\\qquad l_{ef}=\\mu L,\\qquad \\lambda=\\frac{l_{ef}}{i} $$", ""])
    if not have_results:
        return lines + ["**PENDING:** расчётные длины/гибкости ещё не сформированы выполненным runtime. Раздел не считается проверенным до появления trace-bound результатов.", ""]
    for axis, bundle in (("z", z), ("y", y)):
        parts = []
        for label, key in ((f"μ_{axis}", "mu"), (f"l_{{ef,{axis}}}", "lef"), (f"i_{axis}", "i"), (f"λ_{axis}", "lam"), (f"λ̄_{axis}", "bar"), (f"φ_{axis}", "phi")):
            if bundle[key]:
    ...
    if governing:
        ...
        lines.extend(["Определяющая ось отдельно не выводится presentation-слоем без явного runtime-результата; значения по обеим осям приведены выше.", ""])
...
def _section_sp16_v090(report: Mapping[str, Any]) -> list[str]:
    lines = ["## 3. Проверка несущей способности по СП 16", "", "В основной текст включаются расчётно значимые проверки и их trace-bound результаты. Служебная маршрутизация и внутренние ID остаются в Audit.", ""]
    ...
    if mech_pass and isinstance(_v089._row_raw(mech_pass), bool):
        lines.extend(["**Инженерный вывод:** применимые механические проверки СП 16 " + ("выполнены." if _v089._row_raw(mech_pass) else "не выполнены."), ""])
    elif not any((compression_block, gov_eta)):
        lines.extend(["**PENDING:** определяющий результат механической проверки ещё не сформирован runtime.", ""])
...
def _fatigue_section(report: Mapping[str, Any], evidence: Mapping[str, Any]) -> list[str]:
    ...
    if general:
        ...
        lines.extend([
            ...
            f"$α$ = **{_num(alpha)}**; $ρ=σ_{{min}}/σ_{{max}}$ = **{_num(rho)}**; по таблице 36 $γ_v$ = **{_num(gamma_v)}**.", "",
        ...
        if all(v is not None for v in (sigma_max, alpha, rv, gamma_v, eta)):
            lines.extend([f"Подстановка trace-bound значений: $|σ_{{max}}|={_num(abs(float(sigma_max)))}$ МПа, $α={_num(alpha)}$, $R_v={_num(rv)}$ МПа, $γ_v={_num(gamma_v)}$; runtime-результат $η_{{170}}$ = **{_num(eta)}**.", ""])
        cap = general.get("fatigue_resistance_cap_check")
        if isinstance(cap, Mapping):
            cap_pass = cap.get("pass")
            lines.extend([f"Ограничение $αR_vγ_v\\le R_u/γ_u$: коэффициент использования **{_num(cap.get('cap_utilization'))}**, результат **{'PASS' if cap_pass is True else 'FAIL' if cap_pass is False else '—'}**.", ""])
...
def _brittle_section(report: Mapping[str, Any], evidence: Mapping[str, Any]) -> list[str]:
        ...
        if lam.get("required") is False:
            ...
            if factors:
                lines.extend([
                    ...
                    f"Runtime-результат формулы (174): **{_num(lam.get('equation_174_base_risk_percent'))} %**; после применимого воздействия: **{_num(lam.get('adjusted_risk_percent'))} %**.", "",
                ])
            ...
            if required_z is not None or selected_z is not None:
                lines.extend([f"Требуемая Z-группа: **{_num(required_z)}**; принятая по сертификату: **{_num(selected_z)}**; достаточность: **{'PASS' if lam.get('selected_quality_meets_minimum_group') is True else 'FAIL' if lam.get('selected_quality_meets_minimum_group') is False else '—'}**.", ""])
    ...
    lines.extend(["*Нормативное основание: СП 16.13330.2017, раздел 13, пп. 13.1–13.5, формула (174), таблица 37.*", ""])
    return lines


def _section_fire_mechanics_v090(report: Mapping[str, Any]) -> list[str]:
    lines = ["## 4. Расчёт при пожаре по СП 554", "", "### 4.1 Передача результатов СП 16 → СП 554", ""]
    mech_pass = _v089._find_row(report, qids=("SP16_MECHANICAL_PASS",), names=("sp16_mechanical_pass", "status механического gate"))
    ...
    if mech_pass and isinstance(_v089._row_raw(mech_pass), bool):
        lines.extend(["Пожарная ветвь использует квалифицированное механическое состояние, сформированное после применимых проверок СП 16. Результаты не пересчитываются и не заменяются упрощённым набором усилий.", "", f"Механический gate СП 16: **{'PASS' if _v089._row_raw(mech_pass) else 'FAIL'}**.", ""])
        if eta:
            lines.extend([f"Определяющий коэффициент использования СП 16: **{_v089._display(eta)}**.", ""])
    else:
        lines.extend(["**PENDING:** квалифицированный механический handoff СП 16 ещё не сформирован; пожарная механика не должна трактоваться как завершённая.", ""])
...
def _section_conclusion_v090(report: Mapping[str, Any]) -> list[str]:
    ...
    if compliance_hit and fact_hit and rreq:
        ...
        lines.extend([
            ...
            "**Итог:** " + (f"требование **{designation}** выполняется." if passed and designation else f"требуемый предел **{designation}** не обеспечен." if (not passed and designation) else "требование по огнестойкости выполняется." if passed else "требование по огнестойкости не выполняется."), "",
        ])
        if designation:
            lines.extend([f"### Результат: **{designation} — {'PASS' if passed else 'FAIL'}**", ""])
    else:
        lines.extend(["**PENDING:** явный runtime-результат соответствия $R_{fact}$ требуемому пределу ещё отсутствует. Presentation-слой не выводит PASS/FAIL из самостоятельного сравнения чисел.", ""])
    return lines
...
def render_expertise_narrative_markdown_v090(report: Mapping[str, Any]) -> str:
    lines = [
        ...
        "Основной отчёт построен по инженерным разделам из выполненного trace. Внутренняя маршрутизация, нулевые компоненты силового состояния и технические ID вынесены в Audit.", "",
    ]
    for builder in (_section_inputs_v090, _section_material_v090, _section_sp16_v090, _section_fire_mechanics_v090, _v089._section_thermal, _section_conclusion_v090):
...
__all__ = [
    "install", "render_expertise_narrative_markdown_v090", "_engineering_load_rows",
    "_section_inputs_v090", "_section_material_v090", "_slenderness_section",
    "_section_sp16_v090", "_section_fire_mechanics_v090", "_section_conclusion_v090",
    "_fatigue_section", "_brittle_section",
]
