...
Presentation only.  This layer never calculates engineering results.  It fixes
regressions exposed by an interactive run after v0.93:

* future SP16/SP554/thermal/conclusion chapters are removed until completed
  evidence for that stage exists;
* executed section properties already humanised by REPORT-IR v0.92 are shown
  as an engineering-readable profile card;
* Table-2 design resistances are not presented as completed before gamma_m is
  available; once gamma_m exists the normative relation and trace values are
  shown explicitly as formula -> substitution -> recorded result.
"""
...
_MECH_OWNER_TOKENS = (
    "SP16_MECH", "SP16_I_AMBIENT", "SP16_APPLICABILITY", "MECH7", "MECH8", "MECH9",
    "FATIGUE", "BRITTLE", "TABLE32", "SLENDERNESS", "EFFECTIVE_LENGTH",
)
_FIRE_OWNER_TOKENS = ("FIRE-BRIDGE", "SP554_C_", "SP554_FIRE", "FIRE_MECH", "FIRE-D2")
_THERMAL_OWNER_TOKENS = (
    "THERMAL", "UNPROTECTED", "PROTECTED", "FIRE-D3", "SP554_R_", "SP554_12", "PROTECTION",
)
...
def _blocks(report: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    return [
        ...
        if isinstance(block, Mapping) and block.get("progress_state") != PENDING_CURRENT_STATE
    ]
...
def _owner_ids(report: Mapping[str, Any]) -> tuple[str, ...]:
    return tuple(str(block.get("owner_node_id") or "").upper() for block in _blocks(report))
...
def _stage_visibility(report: Mapping[str, Any]) -> dict[int, bool]:
    mechanics = _has_owner_token(report, _MECH_OWNER_TOKENS) or _has_qid(
        report, "SP16_MECHANICAL_PASS", "sp16_effective_length_zy_trace", "governing_utilization"
    )
    fire = _has_owner_token(report, _FIRE_OWNER_TOKENS) or _has_qid(
        report, "gamma_T_required", "gamma_E_required", "gamma_e_compression", "critical_temperature_c"
    )
    thermal = _has_owner_token(report, _THERMAL_OWNER_TOKENS) or _has_qid(
        report, "R_unprot", "R_prot", "R_fact", "thermal_unprotected_trace", "thermal_protected_trace"
    )
    conclusion = _has_owner_token(report, _RESULT_OWNER_TOKENS) or _has_qid(
        report, "R_req", "fire_resistance_pass", "R_required", "R_fact_vs_R_req"
    )
...
def _prune_future_chapters(markdown: str, report: Mapping[str, Any]) -> str:
    ...
    for number in (6, 5, 4, 3):
        if not visibility[number]:
            text = _CHAPTER_RE[number].sub("", text, count=1)
    return text
...
def _material_chapter(report: Mapping[str, Any]) -> str:
    ...
    if ryn or run or (gm_ready and (ry or ru or rs)):
        ...
        # Design values are reportable only after gamma_m exists.  The renderer
        # formats operands/results already recorded in REPORT-IR; it never derives
        # a missing engineering value.
        if gm_ready and ryn and ry:
            lines.extend([
                "Расчётное сопротивление по пределу текучести:", "",
                "$$ R_y=\\frac{R_{yn}}{\\gamma_m} $$", "",
                f"$$ R_y=\\frac{{{_fmt(_raw(ryn))}}}{{{_fmt(gm)}}}={_fmt(_raw(ry))}\\;\\mathrm{{MPa}} $$", "",
            ])
        if gm_ready and run and ru:
            lines.extend([
                "Расчётное сопротивление по временному сопротивлению:", "",
                "$$ R_u=\\frac{R_{un}}{\\gamma_m} $$", "",
                f"$$ R_u=\\frac{{{_fmt(_raw(run))}}}{{{_fmt(gm)}}}={_fmt(_raw(ru))}\\;\\mathrm{{MPa}} $$", "",
            ])
        if gm_ready and rs:
...
def _replace_material(markdown: str, report: Mapping[str, Any]) -> str:
    ...
    if pattern.search(markdown):
        return pattern.sub((chapter + "\n\n") if chapter else "", markdown, count=1)
    if not chapter:
        return markdown
...
def _replace_profile_section(markdown: str, report: Mapping[str, Any]) -> str:
    ...
    if not section:
        return markdown
    text = re.sub(r"(?ms)^### 1\.1\s.*?(?=^### 1\.2\s|^## 2\.\s|\Z)", "", markdown, count=1)
