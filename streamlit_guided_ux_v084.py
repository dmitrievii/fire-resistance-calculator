...
def _svg_chart(series: Sequence[Mapping[str, Any]], *, x_key: str, y_key: str, group_key: str,
               width: int = 780, height: int = 410, x_label: str = "t, мин", y_label: str = "T, °C",
               actual_group: str | None = None, fire_group: str | None = None,
               guide_x: float | None = None, guide_y: float | None = None) -> str:
    ...
    if guide_y is not None:
        ymin, ymax = min(ymin, guide_y), max(ymax, guide_y)
    if xmax <= xmin:
        xmax = xmin + 1.0
    if ymax <= ymin:
        ymax = ymin + 1.0
...
def _render_fdm_visualizations(core: Any, env: Mapping[str, Any]) -> None:
    ...
    with t2:
        ...
        for prof in vis.get('profile_samples') or []:
            ...
            for x,y in zip(prof.get('positions_mm') or [],prof.get('temperatures_c') or []):
                rows.append({'x':float(x),'y':float(y),'series':name})
        if rows:
            st.markdown(_svg_chart(rows,x_key='x',y_key='y',group_key='series',x_label='Координата по толщине защиты, мм',y_label='T, °C'),unsafe_allow_html=True)
    with t3:
        ...
        if rows:
            st.markdown(_svg_chart(rows,x_key='x',y_key='y',group_key='series',x_label='t, мин',y_label='T, °C',actual_group='Сталь',fire_group='ISO 834 / ГОСТ 30247.0'),unsafe_allow_html=True)
...
def install(core: Any) -> None:
    ...
    def _render_generic(card,old_payload,old_provenance,mode_key):
        ...
        if old_payload is None and nid==_JOINTS_NODE:
            old_payload=False
        if old_payload is None and nid==_INITIAL_TEMP_NODE:
            old_payload=20.0
        if old_payload is None and nid==_VALIDATION_NODE:
            old_payload={"protection_12_6_max_deviation_pct":20.0,"protection_12_6_validation_evidence_complete":True}
