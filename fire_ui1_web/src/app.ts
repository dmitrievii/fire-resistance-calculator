...
const els = {
  ...
  next: $("next-button") as HTMLButtonElement,
  back: $("back-button") as HTMLButtonElement,
  cancelEdit: $("cancel-edit") as HTMLButtonElement,
  ...
  ledgerSearch: $("ledger-search") as HTMLInputElement,
  ledgerFilter: $("ledger-filter") as HTMLSelectElement,
  message: $("message-area"),
  footer: $("normative-footer"),
  newSession: $("new-session") as HTMLButtonElement,
  saveSession: $("save-session") as HTMLButtonElement,
  loadSession: $("load-session") as HTMLInputElement,
};

let contract: Dict | null = null;
let envelope: SessionEnvelope | null = null;
let draftPayload: any = undefined;
let draftProvenance: Dict | null = null;
let submitInFlight = false;
let historyCursor: number | null = null;
let catalogFamilies: Dict[] | null = null;
let materialStrengthCatalogCache: Dict | null = null;
let activeTab: "ledger" | "trace" = "ledger";
...
function formatNumber(value: number): string {
  ...
  if ((abs !== 0 && abs < 1e-4) || abs >= 1e7) return value.toExponential(5);
...
function cardByNodeId(nodeId: string): Dict | null {
  if (!contract) return null;
  return (contract.node_cards as Dict[]).find((c) => c.node_id === nodeId) ?? null;
}

function currentRenderCard(): Dict | null {
...
function currentHistoryRow(): Dict | null {
  if (!envelope || historyCursor === null) return null;
...
function statusLabel(status: string): string {
  if (status === "AWAITING_INPUT") return "Ожидается ответ";
  if (status === "RESULT") return "Результат маршрута";
  if (status === "COMPLETE") return "Расчёт завершён";
  if (status.startsWith("BLOCKED_EXECUTOR_REQUIRED")) return "Ожидается producer binding (UI2)";
  if (status.startsWith("BLOCKED_MISSING_INPUTS")) return "Не хватает исходных данных";
  if (status.startsWith("BLOCKED")) return "Маршрут остановлен fail-closed";
  return status;
}

function refsHtml(refs: Dict[] = []): string {
...
function optionSource(card: Dict, field: Dict): Dict[] {
  ...
  if (field.data_type === "boolean") return [{ value: true, label: "Да" }, { value: false, label: "Нет" }];
  if (Array.isArray(card.options) && card.options.length) return card.options;
  if (Array.isArray(field.enum_values) && field.enum_values.length) return field.enum_values;
  return [];
}

function selectedValueForReview(): { payload: any; provenance: Dict | null } | null {
...
function reviewChanged(): boolean {
...
function renderField(card: Dict, field: Dict, currentValue: any): string {
...
function sectionFamilyFromShapeGroup(shapeGroup: string): string {
  const map: Dict = {
...
function sectionShapeSvg(shapeGroup: string): string {
  const common = `viewBox="0 0 120 120" class="section-svg" aria-label="${escapeHtml(shapeGroup)}"`;
  if (shapeGroup === "I_ROLLED_DSYMM") return `<svg ${common}><path d="M20 18 H100 V30 H67 V90 H100 V102 H20 V90 H53 V30 H20 Z" fill="none" stroke="currentColor" stroke-width="6"/></svg>`;
  if (shapeGroup === "RHS_SHS") return `<svg ${common}><rect x="20" y="20" width="80" height="80" rx="7" fill="none" stroke="currentColor" stroke-width="7"/><rect x="36" y="36" width="48" height="48" rx="3" fill="none" stroke="currentColor" stroke-width="5"/></svg>`;
  if (shapeGroup === "CHS") return `<svg ${common}><circle cx="60" cy="60" r="42" fill="none" stroke="currentColor" stroke-width="7"/><circle cx="60" cy="60" r="27" fill="none" stroke="currentColor" stroke-width="5"/></svg>`;
  if (shapeGroup === "CHANNEL") return `<svg ${common}><path d="M92 20 H30 V100 H92 M30 20 H92 M30 100 H92" fill="none" stroke="currentColor" stroke-width="8" stroke-linejoin="round"/></svg>`;
  if (shapeGroup === "ANGLE") return `<svg ${common}><path d="M30 20 V92 H100" fill="none" stroke="currentColor" stroke-width="12" stroke-linejoin="miter"/></svg>`;
  if (shapeGroup === "TEE") return `<svg ${common}><path d="M20 25 H100 M60 25 V100" fill="none" stroke="currentColor" stroke-width="10"/></svg>`;
  if (shapeGroup === "Z_SECTION") return `<svg ${common}><path d="M18 24 H96 L24 96 H102" fill="none" stroke="currentColor" stroke-width="8" stroke-linejoin="round"/></svg>`;
  if (shapeGroup === "C_LIPPED_SECTION") return `<svg ${common}><path d="M92 22 H34 V98 H92 M92 22 V40 M92 98 V80" fill="none" stroke="currentColor" stroke-width="7"/></svg>`;
  return `<svg ${common}><path d="M28 25 H92 V95 H28 Z" fill="none" stroke="currentColor" stroke-width="6" stroke-dasharray="8 5"/></svg>`;
}

function renderSectionEditorEntry(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  ...
  els.question.querySelectorAll<HTMLButtonElement>("[data-geometry-mode]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      button.classList.add("selected"); updateNextState();
    });
  });
}

function profilePreviewHtml(resolved: Dict): string {
...
async function renderProfileCatalogSelector(card: Dict, edit: { payload: any; provenance: Dict | null } | null): Promise<void> {
  draftPayload = edit?.payload; draftProvenance = null;
  els.question.innerHTML = `
  ...
  if (!catalogFamilies) { const payload = await api<Dict>("/api/profile-catalog/families"); catalogFamilies = payload.families ?? []; }
  const familySelect = $("catalog-family") as HTMLSelectElement, profileSelect = $("catalog-profile") as HTMLSelectElement;
  familySelect.innerHTML = `<option value="">— выберите семейство —</option>` + (catalogFamilies ?? []).map((f: Dict) => {
  ...
  const oldFamily = Number(edit?.payload?.section_catalog_family_id ?? 0); if (oldFamily) familySelect.value = String(oldFamily);
  const loadProfiles = async (familyId: number, selectOld = false) => {
    profileSelect.disabled = true; profileSelect.innerHTML = `<option>Загрузка…</option>`;
    const data = await api<Dict>(`/api/profile-catalog/families/${familyId}/profiles`);
    profileSelect.innerHTML = `<option value="">— выберите профиль —</option>` + (data.profiles ?? []).map((p: Dict) => `<option value="${escapeHtml(`${p.designation}::${p.source_row_id}`)}">${escapeHtml(p.designation)}</option>`).join("");
    profileSelect.disabled = false;
    if (selectOld && edit?.payload?.section_designation) { const row = Number(edit.payload.section_catalog_source_row_id ?? 0); profileSelect.value = `${edit.payload.section_designation}::${row}`; if (profileSelect.value) await resolveSelection(); }
  };
  const resolveSelection = async () => {
    const familyId = Number(familySelect.value), selected = profileSelect.value;
    if (!familyId || !selected) { draftPayload = undefined; updateNextState(); return; }
    const [designation, rowText] = selected.split("::"), row = Number(rowText);
    const resolved = await api<Dict>(`/api/profile-catalog/resolve?family_id=${familyId}&designation=${encodeURIComponent(designation)}&source_row_id=${row}`);
    const shape = resolved.shape_group as string;
    ...
    $("catalog-preview").innerHTML = profilePreviewHtml(resolved); updateNextState();
  };
  familySelect.addEventListener("change", () => { draftPayload = undefined; $("catalog-preview").innerHTML = ""; if (familySelect.value) void loadProfiles(Number(familySelect.value)); else { profileSelect.disabled = true; profileSelect.innerHTML = `<option>— сначала выберите семейство —</option>`; } updateNextState(); });
  profileSelect.addEventListener("change", () => void resolveSelection()); if (oldFamily) await loadProfiles(oldFamily, true); updateNextState();
}

function renderParametricSectionEditor(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  const old: Dict = edit?.payload ?? {}; draftPayload = edit?.payload; draftProvenance = null;
  ...
  const sel=$("param-family") as HTMLSelectElement; sel.value=old.section_family ?? ""; const box=$("param-fields");
  const config: Dict = {
  ...
  const draw=()=>{ const fam=sel.value, fields=config[fam] ?? []; box.innerHTML=fields.map((r:any)=>`<div class="field-block"><label class="field-label">${escapeHtml(r[1])}</label><input class="number-input" data-param="${r[0]}" type="number" step="any" value="${escapeHtml(old[r[0]] ?? "")}"></div>`).join("");
    const sync=()=>{ if(!fam){draftPayload=undefined;updateNextState();return;} const obj:Dict={section_family:fam}; let ready=true; box.querySelectorAll<HTMLInputElement>("[data-param]").forEach(inp=>{if(inp.value!=="")obj[inp.dataset.param!]=Number(inp.value); else if(inp.dataset.param!=="r_root")ready=false;}); if(fam==="i_section")obj.sp16_i_symmetry_class="double_symmetric"; draftPayload=ready?obj:undefined; updateNextState();};
    box.querySelectorAll("input").forEach(x=>x.addEventListener("input",sync)); sync(); };
  sel.addEventListener("change",()=>{draftPayload=undefined;draw();}); draw();
}

function renderManualSectionPropertiesEditor(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  const old = edit?.payload?.section_geometry_2d ?? {}; draftPayload = edit?.payload; draftProvenance = null;
  ...
  const labels:Dict={A_gross:"A, мм²",J_x:"Jx, мм⁴",J_y:"Jy, мм⁴",W_el_x_pos:"Wx,+, мм³",W_el_x_neg:"Wx,−, мм³",W_el_y_pos:"Wy,+, мм³",W_el_y_neg:"Wy,−, мм³",W_pl_x:"Wpl,x, мм³",W_pl_y:"Wpl,y, мм³",I_shear_gross:"I для среза, мм⁴",S_shear_gross:"S для среза, мм³",I_omega_gross:"Iω, мм⁶",W_omega_gross:"Wω, мм⁴",P_heated_m:"Нагреваемый периметр Π, м"};
  ...
  const fam=$("manual-family") as HTMLSelectElement; fam.value=old.section_family??"other";
  const sync=()=>{const obj:Dict={section_family:fam.value,section_is_box:fam.value==="rhs_shs",section_is_channel:fam.value==="channel",section_is_pipe:fam.value==="chs",sp16_i_symmetry_class:fam.value==="i_section"?"double_symmetric":"not_i"};let ready=true;els.question.querySelectorAll<HTMLInputElement>("[data-manual]").forEach(inp=>{if(inp.value!=="")obj[inp.dataset.manual!]=Number(inp.value);else if(required.includes(inp.dataset.manual!))ready=false;});draftPayload=ready?{section_geometry_2d:obj}:undefined;updateNextState();};
  els.question.querySelectorAll("input").forEach(x=>x.addEventListener("input",sync));fam.addEventListener("change",sync);sync();
}

async function renderDoubleBranchSectionEditor(card: Dict, edit: { payload: any; provenance: Dict | null } | null): Promise<void> {
  const old:Dict=edit?.payload??{}; draftPayload=edit?.payload;draftProvenance=null;
  ...
  if(!catalogFamilies){const payload=await api<Dict>("/api/profile-catalog/families");catalogFamilies=payload.families??[];}
  const axis=$("db-axis") as HTMLSelectElement,spacing=$("db-spacing") as HTMLInputElement,fs=$("db-family") as HTMLSelectElement,ps=$("db-profile") as HTMLSelectElement;axis.value=old.double_branch_symmetry_axis??"y";
  fs.innerHTML=`<option value="">— выберите —</option>`+(catalogFamilies??[]).filter((f:Dict)=>!["C_LIPPED_SECTION","Z_SECTION"].includes(f.shape_group)).map((f:Dict)=>`<option value="${f.family_id}">${escapeHtml(f.caption)}</option>`).join("");
  ...
  const load=async(fid:number,restore=false)=>{ps.disabled=true;const data=await api<Dict>(`/api/profile-catalog/families/${fid}/profiles`);ps.innerHTML=`<option value="">— выберите профиль —</option>`+(data.profiles??[]).map((p:Dict)=>`<option value="${escapeHtml(`${p.designation}::${p.source_row_id}`)}">${escapeHtml(p.designation)}</option>`).join("");ps.disabled=false;if(restore&&old.double_branch_designation){ps.value=`${old.double_branch_designation}::${old.double_branch_source_row_id}`;if(ps.value)await resolve();}};
  const resolve=async()=>{if(!fs.value||!ps.value){selected=null;sync();return;}const [designation,rowText]=ps.value.split("::");const row=Number(rowText);const r=await api<Dict>(`/api/profile-catalog/resolve?family_id=${fs.value}&designation=${encodeURIComponent(designation)}&source_row_id=${row}`);selected={designation,source_row_id:row};$("db-preview").innerHTML=profilePreviewHtml(r);sync();};
  fs.addEventListener("change",()=>{selected=null;$("db-preview").innerHTML="";if(fs.value)void load(Number(fs.value));sync();});ps.addEventListener("change",()=>void resolve());axis.addEventListener("change",sync);spacing.addEventListener("input",sync);
  if(old.double_branch_family_id){fs.value=String(old.double_branch_family_id);await load(Number(fs.value),true);}sync();
}

async function renderMaterialStrengthEditor(card: Dict, edit: { payload: any; provenance: Dict | null } | null): Promise<void> {
  const old: Dict = edit?.payload ?? {};
  draftPayload = edit?.payload;
  draftProvenance = null;
  ...
  if (!materialStrengthCatalogCache) materialStrengthCatalogCache = await api<Dict>("/api/material-strength/catalog");
  const context = envelope ? await api<Dict>(`/api/sessions/${encodeURIComponent(envelope.session_id)}/material-strength-context`) : {};
  const products: Dict[] = materialStrengthCatalogCache?.products ?? [];
  const product = $("mat-product") as HTMLSelectElement;
  const grade = $("mat-grade") as HTMLSelectElement;
  const interval = $("mat-interval") as HTMLSelectElement;
  const preview = $("mat-preview");
  const productHint = $("mat-product-hint");
  const thicknessHint = $("mat-thickness-hint");

  product.innerHTML = `<option value="">— выберите —</option>` + products.map((p: Dict) => `<option value="${escapeHtml(p.value)}">${escapeHtml(p.label)} · СП16 ${escapeHtml(p.source_table)}</option>`).join("");
  const initialProduct = old.steel_product_form ?? context.suggested_product_form ?? "";
  if (initialProduct) product.value = initialProduct;
  ...
  const selectedProduct = (): Dict | null => products.find((p: Dict) => p.value === product.value) ?? null;
  const selectedGrade = (): Dict | null => selectedProduct()?.grades?.find((g: Dict) => g.steel_grade === grade.value) ?? null;
  const selectedInterval = (): Dict | null => selectedGrade()?.intervals?.find((i: Dict) => i.interval_key === interval.value) ?? null;
  const contains = (row: Dict, t: number): boolean => {
    const x = row.thickness_interval ?? {};
    if (x.min_mm != null && (t < Number(x.min_mm) || (t === Number(x.min_mm) && !x.min_inclusive))) return false;
    if (x.max_mm != null && (t > Number(x.max_mm) || (t === Number(x.max_mm) && !x.max_inclusive))) return false;
    return true;
  };

  const renderPreview = () => {
    const p = selectedProduct(), r = selectedInterval();
    if (!p || !r) { preview.innerHTML = ""; draftPayload = undefined; updateNextState(); return; }
    ...
    const obj: Dict = { steel_product_form: product.value, steel_grade: grade.value, steel_strength_interval_key: interval.value };
    if (hasExactThickness && context.suggested_product_form === product.value && contains(r, exactThickness)) obj.governing_product_thickness_mm = exactThickness;
    draftPayload = obj;
    updateNextState();
  };
  ...
  const fillIntervals = (preserve = "") => {
    ...
    if (!g) { interval.disabled = true; interval.innerHTML = `<option value="">— сначала выберите марку —</option>`; preview.innerHTML = ""; draftPayload = undefined; updateNextState(); return; }
    interval.disabled = false;
    interval.innerHTML = `<option value="">— выберите диапазон —</option>` + (g.intervals ?? []).map((r: Dict) => `<option value="${escapeHtml(r.interval_key)}">${escapeHtml(r.interval_label)}</option>`).join("");
    if (preserve && (g.intervals ?? []).some((r: Dict) => r.interval_key === preserve)) interval.value = preserve;
    else if (hasExactThickness && context.suggested_product_form === product.value) {
      const match = (g.intervals ?? []).find((r: Dict) => contains(r, exactThickness));
      if (match) interval.value = match.interval_key;
    }
    renderPreview();
  };
  ...
  const fillGrades = (preserveGrade = "", preserveInterval = "") => {
    ...
    if (!p) { grade.disabled = true; grade.innerHTML = `<option value="">— сначала выберите вид проката —</option>`; fillIntervals(); return; }
    grade.disabled = false;
    grade.innerHTML = `<option value="">— выберите марку стали —</option>` + (p.grades ?? []).map((g: Dict) => `<option value="${escapeHtml(g.steel_grade)}">${escapeHtml(g.steel_grade)}</option>`).join("");
    if (preserveGrade && (p.grades ?? []).some((g: Dict) => g.steel_grade === preserveGrade)) grade.value = preserveGrade;
...
function renderSectionWeakeningEditor(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  ...
  const renderDetails = (enabled: boolean) => {
    if (!enabled) {
      details.innerHTML = "";
    ...
    const sync = () => {
      const d = Number((document.getElementById("weak-d") as HTMLInputElement).value);
      const pitch = Number((document.getElementById("weak-pitch") as HTMLInputElement).value);
      const tRaw = (document.getElementById("weak-thickness") as HTMLInputElement).value;
      const t = tRaw === "" ? undefined : Number(tRaw);
      const hint = document.getElementById("weak-validation") as HTMLElement;
      if (!Number.isFinite(d) || d <= 0) { hint.textContent = "Диаметр d должен быть больше 0."; draftPayload = undefined; }
      else if (!Number.isFinite(pitch) || pitch <= d) { hint.textContent = "Для формулы (45) требуется s > d."; draftPayload = undefined; }
      else if (t !== undefined && (!Number.isFinite(t) || t <= 0)) { hint.textContent = "Если tₕ задана вручную, она должна быть больше 0."; draftPayload = undefined; }
      else {
        hint.textContent = "";
        ...
        if (t !== undefined) draftPayload.local_thickness_mm = t;
      }
      updateNextState();
    };
    details.querySelectorAll("input").forEach(x => x.addEventListener("input", sync)); sync();
  };
  els.question.querySelectorAll<HTMLButtonElement>("[data-weak]").forEach(b => b.addEventListener("click", () => {
    els.question.querySelectorAll("[data-weak]").forEach(x => x.classList.remove("selected")); b.classList.add("selected"); renderDetails(b.dataset.weak === "holes");
  }));
  renderDetails(has);
}

function renderCanonicalLoadMenu(card: Dict, edit: { payload: any; provenance: Dict | null } | null, kind: "ambient" | "fire"): void {
  draftProvenance = null;
  const old = edit?.payload ?? {};
  const kN = (v: any) => Number(v ?? 0) / 1000;
  const kNm = (v: any) => Number(v ?? 0) / 1e6;
  const first = (...vals:any[]) => vals.find(v => v !== undefined && v !== null) ?? 0;
  ...
  els.question.innerHTML = `
    ...
    ${refsHtml(card.normative_refs)}`;
  const sync = () => {
    const read=(id:string)=>Number((document.getElementById(id) as HTMLInputElement).value || 0);
    ...
    if (vals.some(v=>!Number.isFinite(v))) { draftPayload=undefined; updateNextState(); return; }
    const combo = {
    ...
    (document.getElementById("load-live-summary") as HTMLElement).innerHTML=`<strong>Активно:</strong> ${active.join(" · ") || "нулевые усилия"}${Qy!==0||T!==0?" · есть fail-closed component":""}`;
    updateNextState();
  };
  els.question.querySelectorAll("input").forEach(x=>x.addEventListener("input",sync)); sync();
}

function renderAmbientSignedLoadMenu(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  renderCanonicalLoadMenu(card, edit, "ambient");
}

function renderFireSignedLoadMenu(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  renderCanonicalLoadMenu(card, edit, "fire");
}

function renderBimomentDirectInput(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  draftProvenance = null;
  ...
  els.question.innerHTML = `
    ...
    ${refsHtml(card.normative_refs)}`;
  const input = document.getElementById("bimoment-B") as HTMLInputElement;
  const sync=()=>{ const b=Number(input.value); draftPayload=Number.isFinite(b)?b*1e9:undefined; updateNextState(); };
  input.addEventListener("input",sync); sync();
}

function renderVerificationPlanSummary(card: Dict): void {
  const plan = envelope?.state?.values?.verification_plan?.value ?? {};
  const branches: Dict[] = plan.branches ?? [];
  draftPayload = true; draftProvenance = null;
  const stateLabels: Dict = { central_tension:"центральное растяжение", central_compression:"центральное сжатие", bending:"изгиб", compression_plus_bending:"сжатие + изгиб", tension_plus_bending:"растяжение + изгиб" };
  els.question.innerHTML = `
    ...
    <div class="plan-list">${branches.map((b:Dict)=>`<div class="result-card"><strong>${escapeHtml(b.label ?? b.id)}</strong><div class="field-hint">${b.status === "deferred" ? "DEFERRED — ветвь будет отмечена явно и не остановит остальные проверки." : "Будет проверяться."}${b.note?` ${escapeHtml(b.note)}`:""}</div></div>`).join("")}</div>
...
function renderApplicabilityCensusSummary(card: Dict): void {
  const census = envelope?.state?.values?.sp16_applicability_census?.value ?? {};
  const active: Dict[] = census.active_checks ?? [];
  const contextual: Dict[] = census.context_driven_checks ?? [];
  draftPayload = true; draftProvenance = null;
  const statusLabel=(row:Dict)=>{
    const st=String(row.runtime_status ?? "");
    if(st.startsWith("deferred_")) return "DEFERRED / FAIL-CLOSED";
    if(st === "direct_input_partial_runtime") return "PARTIAL — B задан напрямую";
    return "APPLICABLE";
  };
  els.question.innerHTML = `
    ...
    <div class="plan-list">${active.map((r:Dict)=>`<div class="result-card"><strong>${escapeHtml(r.label ?? r.id)}</strong><div class="field-hint">${escapeHtml(statusLabel(r))} · ${escapeHtml(r.reason ?? "")}${r.normative_scope?.length?` · ${escapeHtml(r.normative_scope.join(", "))}`:""}</div></div>`).join("") || `<div class="result-card"><strong>Нет action-driven проверок</strong><div class="field-hint">Нулевое силовое состояние.</div></div>`}</div>
    ${contextual.length?`<div class="callout"><strong>Контекстные проверки, которые нельзя решить только по N/M/Q/T:</strong><br>${contextual.map((r:Dict)=>`${escapeHtml(r.label ?? r.id)} — ${escapeHtml(r.trigger ?? "")}`).join("<br>")}</div>`:""}
...
function renderRequiredRSelector(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  ...
  els.question.querySelectorAll<HTMLButtonElement>("[data-r]").forEach(b=>b.addEventListener("click",()=>{draftPayload=Number(b.dataset.r);els.question.querySelectorAll("[data-r]").forEach(x=>x.classList.remove("selected"));b.classList.add("selected");(document.getElementById("r-custom") as HTMLInputElement).value="";updateNextState()}));
  const custom=document.getElementById("r-custom") as HTMLInputElement;custom.addEventListener("input",()=>{draftPayload=custom.value===""?undefined:Number(custom.value);els.question.querySelectorAll("[data-r]").forEach(x=>x.classList.remove("selected"));updateNextState()});
}


function renderTable30SchemeSelector(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  const current = edit?.payload;
  draftPayload = current;
  draftProvenance = null;
  const rows: Dict[] = [
  ...
  const selectValue=(v:string)=>{ draftPayload=v; els.question.querySelectorAll("[data-table30]").forEach(x=>x.classList.toggle("selected",(x as HTMLElement).dataset.table30===v)); updateNextState(); };
  els.question.querySelectorAll<HTMLButtonElement>("[data-table30]").forEach(b=>b.addEventListener("click",()=>selectValue(b.dataset.table30!)));
  const left=document.getElementById("mu-left") as HTMLSelectElement;
  const right=document.getElementById("mu-right") as HTMLSelectElement;
  const note=document.getElementById("mu-shortcut-note")!;
  const sync=()=>{
    const a=left.value,b=right.value; let v="";
    if(a==="hinged"&&b==="hinged") v="S1";
    else if((a==="fixed"&&b==="hinged")||(a==="hinged"&&b==="fixed")) v="S2";
    else if(a==="fixed"&&b==="fixed") v="S3";
    else if((a==="fixed"&&b==="free")||(a==="free"&&b==="fixed")) v="S4";
    if(v){ const row=rows.find(x=>x.value===v)!; note.textContent=`Распознано: ${row.label}; μ = ${row.mu}.`; selectValue(v); }
    else if(a&&b){ note.textContent="Эта пара концов не даёт однозначной классической схемы S1–S4. Выберите нормативную карточку вручную."; }
  };
  left.addEventListener("change",sync); right.addEventListener("change",sync);
  updateNextState();
}

function renderTable7CurveSelector(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  const current=edit?.payload;
  draftPayload=current; draftProvenance=null;
  const rows:Dict[]=[
  ...
  els.question.querySelectorAll<HTMLButtonElement>("[data-table7]").forEach(b=>b.addEventListener("click",()=>{draftPayload=b.dataset.table7;els.question.querySelectorAll("[data-table7]").forEach(x=>x.classList.toggle("selected",(x as HTMLElement).dataset.table7===draftPayload));updateNextState()}));
  updateNextState();
}



function renderTable21TypeSelector(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  const current = edit?.payload;
  draftPayload = current;
  draftProvenance = null;
  const options: Dict[] = card.options ?? [];
  const extra: Dict = {
  ...
  els.question.innerHTML = `<div class="card-kicker"><span class="standard-badge">СП 16 · табл. 21</span><span>9.2.5</span></div>
    ...
    <div class="option-grid">${options.map((o:Dict)=>`<button type="button" class="option-card ${current===o.value?"selected":""}" data-table21="${escapeHtml(String(o.value))}">
    ...
    ${refsHtml(card.normative_refs)}`;
  els.question.querySelectorAll<HTMLButtonElement>("[data-table21]").forEach(b=>b.addEventListener("click",()=>{
    draftPayload=b.dataset.table21;
    els.question.querySelectorAll("[data-table21]").forEach(x=>x.classList.toggle("selected",(x as HTMLElement).dataset.table21===draftPayload));
    updateNextState();
  }));
  updateNextState();
}

function renderHeatedSidesSelector(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  const current = edit?.payload;
  draftPayload = current;
  draftProvenance = null;
  const options: Dict[] = [
  ...
  els.question.querySelectorAll<HTMLButtonElement>("[data-heated-sides]").forEach(b=>b.addEventListener("click",()=>{
    draftPayload=b.dataset.heatedSides;
    els.question.querySelectorAll("[data-heated-sides]").forEach(x=>x.classList.toggle("selected",(x as HTMLElement).dataset.heatedSides===draftPayload));
    updateNextState();
  }));
  updateNextState();
}

function renderProtectionPerimeterModeSelector(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  const current=edit?.payload;
  draftPayload=current; draftProvenance=null;
  const options: Dict[]=[
  ...
  els.question.querySelectorAll<HTMLButtonElement>("[data-prot-perimeter]").forEach(b=>b.addEventListener("click",()=>{
    draftPayload=b.dataset.protPerimeter;
    els.question.querySelectorAll("[data-prot-perimeter]").forEach(x=>x.classList.toggle("selected",(x as HTMLElement).dataset.protPerimeter===draftPayload));
    updateNextState();
  }));
  updateNextState();
}
...
function renderDocumentedPerformanceMatrixEditor(card: Dict, edit: { payload: any; provenance: Dict | null } | null): void {
  const ledgerValue = (qid: string): any => (envelope?.state?.ledger ?? []).find((x: Dict) => x.quantity_id === qid)?.value;
  const current: Dict = (edit?.payload && typeof edit.payload === "object" && !Array.isArray(edit.payload)) ? edit.payload : {};
  const axisText = (v: any): string => Array.isArray(v) ? v.join(", ") : "";
  const matrixText = (v: any): string => Array.isArray(v) ? v.map((r:any)=>Array.isArray(r)?r.join(", "):"").join("\n") : "";
  ...
  const sync=()=>{
    const tcr=Number((document.getElementById("perf-tcr") as HTMLInputElement).value);
    const da=parseVector((document.getElementById("perf-delta-axis") as HTMLInputElement).value);
    const pa=parseVector((document.getElementById("perf-prot-axis") as HTMLInputElement).value);
    const mx=parseMatrix((document.getElementById("perf-matrix") as HTMLTextAreaElement).value);
    const confirmed=(document.getElementById("perf-doc-confirm") as HTMLInputElement).checked;
    const carry=(document.getElementById("perf-upper-carry") as HTMLInputElement).checked;
    const status=document.getElementById("perf-editor-status") as HTMLElement;
...
function renderInteractiveCard(card: Dict): void {
  const edit = historyCursor !== null ? selectedValueForReview() : null;
  const component = card.presentation?.component;
  if (component === "section_editor_entry") { renderSectionEditorEntry(card, edit); updateNextState(); return; }
  if (component === "profile_catalog_selector") { void renderProfileCatalogSelector(card, edit); return; }
  if (component === "parametric_section_editor") { renderParametricSectionEditor(card, edit); return; }
  if (component === "manual_section_properties_editor") { renderManualSectionPropertiesEditor(card, edit); return; }
  if (component === "double_branch_section_editor") { void renderDoubleBranchSectionEditor(card, edit); return; }
  if (component === "material_strength_editor") { void renderMaterialStrengthEditor(card, edit); return; }
  if (component === "section_weakening_editor") { renderSectionWeakeningEditor(card, edit); updateNextState(); return; }
  if (component === "ambient_signed_load_menu") { renderAmbientSignedLoadMenu(card, edit); updateNextState(); return; }
  if (component === "fire_signed_load_menu") { renderFireSignedLoadMenu(card, edit); updateNextState(); return; }
  if (component === "bimoment_direct_input") { renderBimomentDirectInput(card, edit); updateNextState(); return; }
  if (component === "verification_plan_summary") { renderVerificationPlanSummary(card); return; }
  if (component === "applicability_census_summary") { renderApplicabilityCensusSummary(card); return; }
  if (component === "table30_scheme_selector") { renderTable30SchemeSelector(card, edit); return; }
  if (component === "table7_curve_selector") { renderTable7CurveSelector(card, edit); return; }
  if (component === "table21_type_selector") { renderTable21TypeSelector(card, edit); return; }
  if (component === "required_r_selector") { renderRequiredRSelector(card, edit); updateNextState(); return; }
  if (component === "heated_sides_selector") { renderHeatedSidesSelector(card, edit); return; }
  if (component === "protection_perimeter_mode_selector") { renderProtectionPerimeterModeSelector(card, edit); return; }
  if (component === "documented_performance_matrix_editor") { renderDocumentedPerformanceMatrixEditor(card, edit); return; }
  const notes = card.notes ?? {};
  const fields: Dict[] = card.fields ?? [];
  ...
  if (useCards) {
    els.question.querySelectorAll<HTMLButtonElement>(".option-card").forEach((button) => {
      button.addEventListener("click", () => {
    ...
    els.question.querySelectorAll<HTMLInputElement | HTMLSelectElement>("[data-field]").forEach((input) => {
      const sync = () => {
        const qid = input.dataset.field!;
        const field = fields.find((f) => f.quantity_id === qid)!;
        let val: any = input.value;
        if (field.data_type === "number") val = input.value === "" ? undefined : Number(input.value);
        if (field.data_type === "boolean") val = input.value === "true" ? true : input.value === "false" ? false : undefined;
        if (card.submit_shape === "scalar") draftPayload = val;
        else {
          const obj = (draftPayload && typeof draftPayload === "object" && !Array.isArray(draftPayload)) ? { ...draftPayload } : {};
          if (val === undefined || val === "") delete obj[qid]; else obj[qid] = val;
          draftPayload = obj;
        }
        updateNextState();
      };
  ...
  if (card.external_handoff) {
    const doc = $("prov-document") as HTMLInputElement;
    const ref = $("prov-reference") as HTMLInputElement;
    const syncProv = () => {
...
function renderNonInteractiveCard(card: Dict | null): void {
  ...
  if (card?.node_id === "SP554_R_UI16_DEFERRED_BRANCHES") {
    ...
    const branches: Dict[] = plan.branches ?? [];
    const deferred = branches.filter((b:Dict)=>b.status === "deferred");
    ...
    const done = branches.filter((b:Dict)=>b.status !== "deferred");
    els.question.innerHTML = `<div class="card-kicker"><span class="standard-badge">Mechanical gate</span><span>fail-closed</span></div>
      ...
      ${loadRows}
      ${deferred.length ? `<div class="callout"><strong>Незавершённые ветви:</strong> ${deferred.map((b:Dict)=>escapeHtml(`${b.label ?? b.id}${b.note?` — ${b.note}`:""}`)).join("<br>")}</div>` : ""}
      ${done.length ? `<div class="field-hint"><strong>Остальные ветви плана:</strong> ${done.map((b:Dict)=>escapeHtml(b.label ?? b.id)).join(" · ")}</div>` : ""}
      ${refsHtml(card.normative_refs ?? [])}`;
    return;
  }
  ...
  if (card?.node_id === "SP554_FIRE_D2_R_CRITICAL_TEMPERATURE") {
    ...
    if(cstatus==="AMBIENT_CAPACITY_EXCEEDED"){ headline="Несущая способность не обеспечена при нормальной температуре"; resultText="Определяющая ветвь требует коэффициент > 1; tcr ≤ 20 °C."; }
    ...
    const branch=(name:string,row:any)=>row?`<div class="result-card"><strong>${name}</strong><div class="field-hint">${escapeHtml(row.status ?? "")} · ${row.temperature_c!==null&&row.temperature_c!==undefined?`t = ${escapeHtml(displayValue(row.temperature_c))} °C`:row.temperature_lower_bound_c!==undefined?`t > ${escapeHtml(displayValue(row.temperature_lower_bound_c))} °C`:"без точной температуры"}</div></div>`:"";
...
function payloadIsReady(card: Dict | null): boolean {
  if (!card?.interactive) return false;
  const fields: Dict[] = card.fields ?? [];
  if (draftPayload === undefined || draftPayload === null || draftPayload === "") return false;
  if (card.submit_shape === "object") {
    const req = fields.filter((f) => f.required !== false).map((f) => f.quantity_id);
    if (!draftPayload || typeof draftPayload !== "object") return false;
    if (req.some((qid) => draftPayload[qid] === undefined || draftPayload[qid] === "")) return false;
  }
  if (card.external_handoff) {
    if (!draftProvenance?.source_document || !draftProvenance?.source_reference) return false;
  }
  return true;
}

function updateNextState(): void {
...
function renderHistory(): void {
  const history: Dict[] = envelope?.state?.interaction_history ?? [];
  if (!history.length) {
  ...
  els.history.querySelectorAll<HTMLButtonElement>("[data-history-index]").forEach((button) => {
    button.addEventListener("click", () => enterReview(Number(button.dataset.historyIndex)));
  });
  if (historyCursor === null) els.history.scrollLeft = els.history.scrollWidth;
}

function renderLedger(): void {
  const rows: Dict[] = envelope?.state?.ledger ?? [];
  const search = els.ledgerSearch.value.trim().toLowerCase();
  const filter = els.ledgerFilter.value;
  const visible = rows.filter((row) => {
    if (filter !== "all" && row.source_kind !== filter) return false;
...
function renderTrace(): void {
  const trace: Dict[] = envelope?.state?.trace ?? [];
  if (!trace.length) {
...
function renderTabs(): void {
  document.querySelectorAll<HTMLButtonElement>(".panel-tab").forEach((button) => {
    const tab = button.dataset.tab as "ledger" | "trace";
...
function render(): void {
  ...
  if (card?.interactive) renderInteractiveCard(card);
  else renderNonInteractiveCard(card);
  updateNextState();
}

function enterReview(index: number): void {
  if (!envelope) return;
  const history: Dict[] = envelope.state.interaction_history ?? [];
...
async function submitCurrent(): Promise<void> {
  if (!envelope || submitInFlight) return;
  const card = currentRenderCard();
  if (!payloadIsReady(card)) return;
  setMessage();
  submitInFlight = true;
    ...
    if (historyCursor !== null) {
      ...
      if (!reviewChanged()) {
        if (historyCursor < history.length - 1) {
          historyCursor += 1;
          draftPayload = undefined; draftProvenance = null; render(); return;
        }
        historyCursor = null; draftPayload = undefined; draftProvenance = null; render(); return;
      }
      envelope = await api<SessionEnvelope>(`/api/sessions/${encodeURIComponent(envelope.session_id)}/answers/${encodeURIComponent(row.node_id)}`, { method: "PUT", body });
      historyCursor = null;
    } else {
      envelope = await api<SessionEnvelope>(`/api/sessions/${encodeURIComponent(envelope.session_id)}/answer`, { method: "POST", body });
    }
    draftPayload = undefined;
    draftProvenance = null;
    render();
...
async function newSession(): Promise<void> {
  setMessage();
  try {
    envelope = await api<SessionEnvelope>("/api/sessions", { method: "POST", body: "{}" });
    historyCursor = null;
    draftPayload = undefined;
    draftProvenance = null;
    render();
...
async function loadSession(file: File): Promise<void> {
  ...
  try {
    ...
    envelope = await api<SessionEnvelope>("/api/sessions/restore", { method: "POST", body: JSON.stringify({ snapshot }) });
    historyCursor = null;
    draftPayload = undefined;
    draftProvenance = null;
    render();
...
async function boot(): Promise<void> {
  try {
    contract = await api<Dict>("/api/ui-contract");
    await newSession();
  } catch (error) {
...
els.back.addEventListener("click", () => {
  ...
  if (historyCursor === null) enterReview(history.length - 1);
  else if (historyCursor > 0) enterReview(historyCursor - 1);
});
...
document.querySelectorAll<HTMLButtonElement>(".panel-tab").forEach((button) => {
  button.addEventListener("click", () => {
    activeTab = button.dataset.tab as "ledger" | "trace";
    renderTabs();
  });
});
